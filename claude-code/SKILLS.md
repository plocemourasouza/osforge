...
## Skill 86: AutoRefine v3 (Autonomous Iteration Loop + Meta-optimization + Cross-Domain Transfer)
**Trigger:** Melhorar skill, refinar skill, otimizar, autorefine, iterar sobre skill, autoresearch, loop de melhoria, self-improving, refinar código, otimizar performance, melhorar métrica, "a skill X não funciona bem", "coverage está baixa", "bundle muito grande", "latência alta", meta-otimizar, transferir aprendizado, aplicar padrão de outro projeto.

Loop autônomo de refinamento iterativo inspirado no Karpathy autoresearch. Generalizado para qualquer artefato com métrica mensurável — skills, código, prompts, configs, docs. Core v2: (1) separação verify+guard — verify mede a métrica principal, guard protege contra regressões; (2) memória cross-sessão via osforge-db — persiste hipóteses KEEP/DISCARD, consulta antes de formular novas para nunca repetir falhas; (3) modo genérico — qualquer arquivo com métrica mecânica. v3 adiciona duas camadas inspiradas no HyperAgents (Meta Research): (4) **Meta-otimização** — após 10+ sessões acumuladas no osforge-db, analisa padrões agregados de sucesso/falha (taxa KEEP por tipo de hipótese, correlação tamanho×sucesso, iterações ótimas) e propõe ajustes ao próprio SKILL.md com aprovação do usuário; (5) **Transferência cross-domínio** — hipóteses KEEP com delta ≥1.5% são avaliadas para generalização, marcadas como transfer-candidate com domínio (nextjs-frontend, api-backend, skill-osforge, etc.), e sugeridas como prioritárias em sessões futuras de projetos do mesmo domínio. Maturidade: 🟡 candidate → 🟢 validated (2+ confirmações) → 🔵 proven (3+). As duas ideias se retroalimentam: B gera dados de transferência, A analisa e cristaliza padrões proven como regras de ferro. Sources: `karpathy/autoresearch`, `uditgoenka/autoresearch`, `alfonsograziano/auto-agent`, `facebookresearch/HyperAgents`.
