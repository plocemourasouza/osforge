...
## Skill 86: AutoRefine v2 (Autonomous Iteration Loop)
**Trigger:** Melhorar skill, refinar skill, otimizar, autorefine, iterar sobre skill, autoresearch, loop de melhoria, self-improving, refinar código, otimizar performance, melhorar métrica, "a skill X não funciona bem", "coverage está baixa", "bundle muito grande", "latência alta".

Loop autônomo de refinamento iterativo inspirado no Karpathy autoresearch. Generalizado para qualquer artefato com métrica mensurável — skills, código, prompts, configs, docs. v2 adiciona: (1) separação verify+guard — verify mede a métrica principal, guard protege contra regressões; se verify melhora mas guard falha, tenta REWORK (max 2x) antes de DISCARD; (2) memória cross-sessão via osforge-db — persiste hipóteses KEEP e DISCARD como decisões, consulta antes de formular novas hipóteses para nunca repetir experimentos falhos; (3) modo genérico além de skills — qualquer arquivo com comando que retorna número (test coverage, bundle size, Lighthouse score, p95 latência, accuracy). Budget fixo (default 5, max 10), model dispatch Haiku/Sonnet, TSV logging, crash recovery, convergence detection. Sources: `karpathy/autoresearch`, `uditgoenka/autoresearch`, `alfonsograziano/auto-agent`.
