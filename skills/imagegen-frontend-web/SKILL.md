...
description: "Direção de arte para gerar mockups de sites premium via geração de imagem: uma imagem horizontal separada por seção, variedade de composição de hero e paleta única consistente entre todas as imagens. ACIONE quando: 'gera imagens de referência do design da landing page', 'cria mockups visuais do site seção por seção', 'quero ver como o site ficaria antes de codar', 'gera um comp de design para o dev recriar', 'imagem de inspiração para o hero, pricing ou footer'. Keywords: imagem, image generation, mockup, design reference, comp, landing page, hero, seções, art direction, website concept. Não acione para: escrever código frontend real (high-end-visual-design, gpt-taste), mockups de apps mobile (imagegen-frontend-mobile), boards de marca e logos (brandkit), geração de DESIGN.md (stitch-design-taste)."
---
