---
name: gpt-taste
description: "Award-winning design engineering with advanced GSAP motion: layout randomization, AIDA structure, hero of at most 2-3 lines, bento grids with no empty cells, and ScrollTrigger with pinning, scrubbing, and card stacking. Use when: 'Awwwards-level landing page with scroll animations', 'add GSAP ScrollTrigger with pinned sections', 'bento grid with no empty holes', 'the hero is breaking into 6 lines, fix it', 'scroll telling with text revealing on scroll'. Keywords: GSAP, ScrollTrigger, awwwards, bento grid, AIDA, scroll telling, pinning, scrubbing, hero, marquee. Do NOT use for: premium polish without GSAP (high-end-visual-design), minimalist interfaces (minimalist-ui), DESIGN.md generation for Stitch (stitch-design-taste), generating site mockups via image (imagegen-frontend-web)."
---
