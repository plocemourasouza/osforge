...
description: "Cloud security attack methodology across AWS, Azure, and GCP. Covers credential harvesting (IMDS, ~/.aws, env vars, leaked CI secrets, instance roles), enumeration (pacu, ScoutSuite, Prowler, ROADtools), IAM privilege escalation (PassRole, AssumeRole chains, Lambda/Functions privilege flips, GCP serviceAccountTokenCreator), persistence, data exfiltration (S3/Blob/GCS, snapshots, RDS/CosmosDB/Cloud SQL), cloud-native lateral movement, serverless attacks, Kubernetes-on-cloud (EKS/AKS/GKE to node and metadata), and CSPM evasion (CloudTrail blind spots, GuardDuty mute). Use when the engagement scope is cloud accounts, when you have obtained cloud credentials, or when assessing AWS/Azure/GCP posture. Trigger keywords: cloud pentest, AWS, Azure, GCP, IAM privesc, IMDS, S3, Kubernetes, serverless, CSPM evasion."
---
