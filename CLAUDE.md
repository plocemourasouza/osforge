...
OSForge is **not an application** — it is the source of truth for the user's global Claude Code (`~/.claude/`) and Cursor (`~/.cursor/`) configuration: **174 skills, 27 agents** (orchestrator + 26 specialists), **14 always-on rules, 9 `spec-*` commands, 8 MCP servers**, hooks, and the `osforge-db` SQLite state CLI (now with local vector memory). There is no build, lint, or test suite. The "build" is the deploy.
